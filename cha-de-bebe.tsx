...
export default function ChaDeBebe() {
  ...
  useEffect(() => {
    (async () => {
      setReservations(await loadShared("cha-gift-reservations", {}));
      setLoading(false);
    })();
  }, []);

  const confirmGift = async (giftId, guestName, amount) => {
    const latest = await loadShared("cha-gift-reservations", {});
    const updated = {
      ...latest,
      [giftId]: { name: guestName, amount, date: new Date().toISOString() },
    };
    await saveShared("cha-gift-reservations", updated);
  ...
  const sendRsvp = async () => {
    const latest = await loadShared("cha-rsvps", []);
    latest.push({
      name: rsvpName.trim(),
      people: parseInt(rsvpCount, 10) || 1,
      date: new Date().toISOString(),
    });
    await saveShared("cha-rsvps", latest);
    setRsvpDone(true);
  };
